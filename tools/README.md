...
- a lab-only WGSL static glyph fidelity panel that compares Tac One curve data through Canvas fallback and WebGPU output, with word/size/transform stress presets, even-odd fill parity, and debug overlays.
- a lab-only WGSL animated glyph instance panel that compares transform-only Tac One animation through Canvas fallback and WebGPU output, now driven by one continuous word-body controller instead of preset-specific phase formulas.
- sample-card typography routed through the same Tac One continuous glyph-controller path, so the eight review scenes and the animated panel share one motion grammar.
...
The WebGPU readiness card checks browser capability. The static renderer panel is isolated from the choreography samples: it renders one Tac One word in Canvas and, when available, a sandboxed WebGPU canvas. It is still lab-only and not promoted into `combat_lab`.

## Font Preprocessing

The first font preprocessing spike uses `Tac One` as the impact-font candidate.
...
The script uses local `fontTools`, caches the source font/license, and writes:

- `game/17_points_of_violence/data/typography/font_manifest.json`
- `game/17_points_of_violence/data/typography/fonts/tac_one.glyphs.json`
- `game/17_points_of_violence/data/typography/reports/tac_one_inspection.html`
- `game/17_points_of_violence/data/typography/source_fonts/TacOne-Regular.ttf`
- `game/17_points_of_violence/data/typography/source_fonts/OFL.txt`
...
The generated JS bridge wraps the same JSON in a classic script so the lab can consume Tac One from `file://` without `fetch()` or module CORS issues.
...
The `WGSL Static Glyph Renderer` section uses the generated Tac One glyph asset and does not touch the eight choreography samples.

- `Canvas fallback from Tac One curves` draws the selected word directly from the preprocessed contour/curve data.
...
The `WGSL Animated Glyph Renderer` section is updated through Slice 7F. It proves continuous word-body glyph motion plus bounded runtime Bezier/contour deformation without changing `combat_lab`.

- Transform presets: `POW force wave`, `WHIFF ribbon`, `BLOCK plate`, `Finisher curl`, and `Mob pressure stress`.
- Deformation review presets: `POW rubber smear`, `BAM impact dent`, `WHIFF ribbon bend`, `BLOCK accordion plate`, `CRACK spiky fracture`, and `WHAM corkscrew exit`.
- Canvas and WebGPU use the same Tac One contours, same controller output, and the same runtime-deformed `GlyphBody` frame data.
...
- Slice 7F deforms runtime copies of the glyph contours only. The generated Tac One JSON remains unchanged, and turning deformation off restores the Slice 7E transform-only look.
